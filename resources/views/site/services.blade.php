...
@section('content')
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div class="mt-14 grid gap-8 md:grid-cols-2 perspective items-stretch">
            @foreach ($services as $i => $service)
                <div data-tilt="6" class="tilt group relative h-full flex flex-col p-8 rounded-2xl bg-white border border-gray-100 shadow-sm hover:shadow-xl transition reveal reveal-delay-{{ ($i % 3) + 1 }}">
                    ...
                    <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-brand-600 to-accent-500 text-white grid place-items-center text-lg font-bold shadow-md">
                    ...
                    <p class="mt-3 text-gray-600 leading-relaxed flex-grow">{{ $service->description }}</p>
                    @if ($service->type === 'animation' && $service->external_url)
                        <a href="{{ $service->external_url }}" target="_blank" rel="noopener"
                           class="shine mt-5 inline-flex items-center gap-1 self-start px-5 py-2.5 rounded-lg bg-brand-600 text-white text-sm font-medium hover:bg-brand-700 transition">
